...
export function DepartmentForm({ departmentId }: { departmentId?: string }) {
  ...
  const [subDepartments, setSubDepartments] = useState<Array<{ id?: number; name: string; isNew?: boolean }>>([])
  ...
  const [createSubDepartment, { loading: createSubLoading }] = useMutation(CREATE_SUB_DEPARTMENT, {
    onError: (error) => {
  ...
  const [deleteSubDepartment, { loading: deleteSubLoading }] = useMutation(DELETE_SUB_DEPARTMENT, {
    onError: (error) => {
  ...
  const isLoading = fetchLoading || createLoading || updateLoading || createSubLoading || deleteSubLoading

  return (
    <Card className="shadow-sm border-0">
      <CardHeader className="pb-2">
        <CardTitle className="text-lg font-medium">{isEditing ? "Edit Department" : "New Department"}</CardTitle>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-xs font-medium">Department Name</FormLabel>
                  <FormControl>
                    <Input placeholder="e.g. Finance" {...field} className="h-9" />
                  </FormControl>
            ...
            <div className="space-y-3">
              <FormItem>
                <FormLabel className="text-xs font-medium">Sub-Departments</FormLabel>
                <div className="flex flex-wrap gap-1.5 mb-2">
                  {subDepartments.length > 0 ? (
                    ...
                    <p className="text-xs text-muted-foreground">No sub-departments added yet</p>
                  )}
                </div>
                <div className="flex gap-2">
                  ...
                  />
                  <Button type="button" variant="outline" className="h-9 text-xs" onClick={addSubDepartment}>
      ...
      <CardFooter className="flex justify-between border-t">
        <Button variant="ghost" onClick={() => router.back()} disabled={isLoading} className="h-8 text-xs">
          Cancel
        </Button>
        <Button onClick={form.handleSubmit(onSubmit)} disabled={isLoading} className="h-8 text-xs">
          {isLoading ? (
            <>
